...
@everywhere function get_FullTimeSeries(time_mins,this_bst0)
    time_Pcounts0=[[] for x in time_mins]
    pmean_time=[0 for x in time_mins]
    for b in this_bst0
        for (it,t) in enumerate(time_mins)
        
            
           if (b.born<=t) & (b.dead>t)
                #print(b.id," ")
                this_pdist=b.plasmid_dist
                out_pdist=[]
                lastNplasmids=-1
                for time_plasmids in this_pdist
                    time_plasmids[1]<=t ? lastNplasmids=time_plasmids[3] : 0
                end
                #push!(out_pdist,lastNplasmids)
                push!(time_Pcounts0[it],lastNplasmids)
            end
        end
    end
    time_Pmean=[]
    for (xi,x1) in enumerate(time_Pcounts0)
        push!(time_Pmean,mean(x1))
    end

return time_Pcounts0,time_Pmean
end
