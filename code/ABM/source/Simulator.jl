...
#@everywhere function simulate(t::Int,bs::Array{Bacteria}, R::Float64, aA::Float64,lid::Int)
# @everywhere function _simulate(t::Int,bs::Array{Bacteria}, R::Float64, aA::Float64,lid::Int)
#    verbose==true ? tic() : 0
...
#         for td=1:rep_delay
#             plasmids_dynamics(bs[i]) 
#         end
        
#         bs[i].plasmids==bs[i].plasmid_dist[end][2:end]  ? 0 : (push!(bs[i].plasmid_dist,[t;bs[i].plasmids])  ) 
            
            
#         if(gATP>maxgATP*rand())
#             bs[i].isDead = antibiotic_dynamics(bs[i],aA)
#             (bs[i].isDead==true) ? (bs[i].dead=t; echo("Bacteria $(i) dead at $(t) "); continue;) : 0
#         end
...
#             pR=round(rand(p_Ndist))
#             pR<0 ? (pR=0;echo("pR 0!")) : 0
#             bs[i].plasmids[2]=deepcopy(pR)

#             pR0=round(rand(p_Ndist))
#             pR0<0 ? (pR0=0; echo("pR0 0!") ) : 0
#             b0.plasmids[2]=deepcopy(pR0)
...
#@everywhere function getInitialPopulation(iniCells::Int64,mu::Float64,phenotype::Int8,variation=true)

# @everywhere function getInitialPopulationParallel(iniCells,mu,phenotype,p_Ndisti,variation=true)
#     b0s= Bacteria[]
#     if(mu==1.)
#         phenotype=0 
#     end
...
#         plasmids=Int[]
#         if(phenotype==1)
#             pR=Int(round(rand(p_Ndisti)))
#             pR<0 ? pR=0 : 0
            
#             plasmids=[Int(mu),pR,pR]
#         else
#             phenotype==-1 ? plasmids=[0,0,0] :  0
#             phenotype==0 ? (plasmids=[0,1,1]) :  0 
#         end
...
#              cATPi=cATP*0.5
#         end
#          b0 = Bacteria(i,phenotype,0,0,Inf,0,cATPi,false,plasmids,deepcopy(gparams),[],[]) 
#         #plasmids_dynamics(b0) 
#         push!(b0.plasmid_dist,[0;b0.plasmids]) 
#         push!(b0s,b0)
#     end
